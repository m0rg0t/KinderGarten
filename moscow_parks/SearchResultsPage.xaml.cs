...
namespace moscow_parks
{
    ...
    /// </summary>
    public sealed partial class SearchResultsPage : moscow_parks.Common.LayoutAwarePage
    {
        ...
        protected override void LoadState(Object navigationParameter, Dictionary<String, Object> pageState)
        {
            ...
            //var filterList = new Filter();
                /*from feed in ViewModelLocator.MainStatic.Items
                ...
                feed.Items.Where(item => (item.Title != null && item.Title.Contains(queryText) ||
                         (item.Content != null && item.Content.Contains(queryText)))),
                          false));*/
            //filterList.Insert(0, new List<ChildPlaceItem>("Все", filterList.SelectMany(f => f.Results), true));

            var filterList = new List<Filter>();
            filterList.Add(new Filter("Все", 0, true));

            this.DefaultViewModel["Results"] = (from item in ViewModelLocator.MainStatic.Items 
                                                where 
                                                    (
                                                    item.Title.Contains(queryText) ||
                                                    item.Address.Contains(queryText)
                                                    )
                                                select item).ToList<ChildPlaceItem>();
        ...
        /// </summary>
        private sealed class Filter : moscow_parks.Common.BindableBase
        {
